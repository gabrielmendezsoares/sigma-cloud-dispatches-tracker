...
const DISPATCHES_PERIOD_DAYS = 2;
...
export const createSigmaCloudEvents = async (): Promise<void> => {  
  try {
    ...
    const sigmaCloudDispatchesTrackerWindowStartedAt = sigmaCloudDispatchesTrackerWindow.started_at;

    await prisma.sigma_cloud_dispatches_tracker_triggers.deleteMany({ where: { updated_at: { lt: sigmaCloudDispatchesTrackerWindowStartedAt } } });
    ...
    const companyMapList = (await sigmaCloudHttpClientInstance.get<ICompanyMap.ICompanyMap[]>('https://api.segware.com.br/v1/companies')).data;
    const startDate = momentTimezone.utc(sigmaCloudDispatchesTrackerWindowStartedAt);
    const endDate = startDate.clone().add(DISPATCHES_PERIOD_MILLISECONDS, 'milliseconds');
    const startDateIsoString = startDate.clone().format('YYYY-MM-DD');
    const endDateIsoString = endDate.clone().format('YYYY-MM-DD');
    const startDateToDate = startDate.clone().toDate();
    const endDateToDate = endDate.clone().toDate();
    ...
    await Promise.allSettled(
      companyMapList.map(
        async (companyMap: ICompanyMap.ICompanyMap): Promise<void> => {
          const dispatchMapList = (await sigmaCloudHttpClientInstance.get<IDispatchMap.IDispatchMap[]>(`https://api.segware.com.br/v1/dispatches?companyId=${ companyMap.id }&startDate=${ startDateIsoString }&endDate=${ endDateIsoString }`)).data;
